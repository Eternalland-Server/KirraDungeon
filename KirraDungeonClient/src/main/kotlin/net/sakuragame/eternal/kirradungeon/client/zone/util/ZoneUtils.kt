...
fun Player.getZoneFee(zone: Zone): MutableMap<String, Double> {
    val emptyFeeData = mutableMapOf<String, Double>()
    if (isOp) return emptyFeeData
    return zone.condition.firstOrNull { player.hasPermission(it.permissionName) }?.feeToTypeMap ?: emptyFeeData
}
